...
- **轻量常驻**：无 GUI 框架、无异步运行时、无控制台窗口；Release EXE 保持在 1 MiB 以内，后台常驻内存仅 ~2 MB。
- **安静反馈**：在当前输入位置附近复用同一个小型状态框，显示“优化中…”和“已复制”。
- **兼容服务**：调用 OpenAI 兼容的 `/chat/completions` 接口，可配置地址、模型和采样参数。
- **内置设置**：在现代化原生界面中管理服务、提示词、热键和应用行为，无需手动编辑 JSON。
...
右键托盘图标，选择 **设置**。设置窗口分为“模型与服务”“优化规则”“应用行为”三页，可管理全部配置项。至少填写 API Key，并确认服务地址和模型名称可用；可先点击 **测试连接** 验证认证、地址和模型，再点击 **保存并应用**。

连接失败时，底部仅显示简短摘要；点击 **查看详情** 可查看服务端返回的完整错误，配置中的 API Key 会自动隐藏。“优化规则”页只显示结果将复制到剪贴板，不再提供无实际选项的结果模式输入框。
...
> `base_url` 应指向 API 根路径，例如 `https://api.openai.com/v1`；程序会自动追加 `/chat/completions`。v1.1.1 及更早版本的重复顶层 API 字段会在加载时自动迁移到当前配置，并在下次保存时清理。不要提交或分享包含真实 API Key 的配置文件。
...
- **设置**：打开原生设置窗口，直接查看、修改、校验并应用全部配置项。
