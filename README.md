...
右键托盘图标，选择 **设置**。设置窗口分为“模型与服务”“优化规则”“应用行为”三页，可管理全部配置项。每套 API 配置可以保存多个模型（每行一个），并通过“当前模型”下拉框选择实际调用的模型。至少填写 API Key，并确认服务地址和当前模型可用；可先点击 **测试连接** 验证认证、地址和模型，再点击 **保存并应用**。
